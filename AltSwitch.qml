// ALT+TAB or SUPER+TAB window list.
//
...
Item {
  ...
  readonly property string modifier: {
    const value = String(pluginEntry.modifier || "ALT").trim().toUpperCase()
    return value === "SUPER" ? "SUPER" : "ALT"
  }
  readonly property string modifierStatePath: {
    const home = Quickshell.env("HOME") || ""
    return home ? home + "/.local/state/omarchy/" + root.pluginId + ".lua" : ""
  }
  readonly property string modifierStateContent:
    "-- Generated by " + root.pluginId + "; edit its modifier setting in shell.json.\n"
    + "return { modifier = \"" + root.modifier + "\" }\n"
  ...
  function setPluginSetting(name, rawValue) {
    if (name === "modifier") {
      const modifier = String(rawValue || "").trim().toUpperCase()
      if (["ALT", "SUPER"].indexOf(modifier) === -1)
        return "modifier must be ALT or SUPER"

      if (!root.updatePluginSetting(name, modifier)) return "unavailable"
      return modifier
    }
  ...
  function publishModifier() {
    if (!root.modifierStatePath) return
    if (modifierStateFile.text() === root.modifierStateContent) return
    modifierStateFile.setText(root.modifierStateContent)
  }

  // shell.json belongs to omarchy-shell and is injected as pluginEntry above.
  // Project only the validated modifier into Lua state that Hyprland can load
  // directly. Reload after the atomic write lands so both sides always agree.
  FileView {
    id: modifierStateFile
    path: root.modifierStatePath
    atomicWrites: true
    printErrors: false
    onLoaded: modifierPublish.restart()
    onLoadFailed: modifierPublish.restart()
  ...
  Timer {
    id: modifierPublish
    interval: 100
    onTriggered: root.publishModifier()
  }

  onModifierChanged: modifierPublish.restart()
  Component.onCompleted: modifierPublish.restart()

  // A switch ends when its configured modifier is released. If that release is
  // ever missed the panel would sit on screen for good, so it also gives up on
  // its own and tells the Hyprland config to reset.
  Timer {
    id: watchdog
    interval: 10000
    onTriggered: {
